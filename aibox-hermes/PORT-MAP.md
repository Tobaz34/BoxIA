...
|---|---|---|---|---|
| Approval-gate | `services/app/src/lib/approval-gate.ts` | `plugins/aibox-approval/` | plugin `pre_tool_call` | ✅ POC |
| Scrub RGPD/PII FR | `services/app/src/lib/pii-scrub.ts` | `plugins/aibox-rgpd/` | plugin `transform_tool_result` | ✅ POC |
| Connecteur Pennylane | `services/connectors/accounting-pennylane/` | `mcp-connectors/pennylane/` | shim MCP (FastMCP) → FastAPI ♻️ | ✅ POC |
...
| Provisioning tenant | `tools/provision-hermes-client.sh` | `provision/provision-tenant.sh` | script idempotent (+ `--check`) | 🟦 |
| Audit | `services/app/src/lib/app-audit.ts` | `plugins/aibox-audit/` | plugin `post_tool_call` → JSONL | ✅ POC |
