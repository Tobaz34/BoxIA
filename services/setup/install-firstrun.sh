...
# ---- 1. Hostname → aibox -----------------------------------------------
c_blue "[1/4] Configuration du hostname → aibox"
hostnamectl set-hostname aibox
grep -q '^127.0.1.1.*aibox' /etc/hosts || echo "127.0.1.1 aibox" >> /etc/hosts

# ---- 2. Avahi (mDNS) ---------------------------------------------------
c_blue "[2/4] Installation Avahi pour aibox.local"
apt-get update -qq
apt-get install -y -qq avahi-daemon avahi-utils libnss-mdns

cp -f "$(dirname "$0")/aibox.avahi.service" /etc/avahi/services/aibox.service
systemctl enable --now avahi-daemon
systemctl reload avahi-daemon || true

# ---- 3. Service systemd firstrun ---------------------------------------
c_blue "[3/4] Installation service systemd"
cp -f "$(dirname "$0")/aibox-firstrun.service" /etc/systemd/system/aibox-firstrun.service
mkdir -p /var/lib/aibox
systemctl daemon-reload
systemctl enable aibox-firstrun.service

# ---- 4. Démarrer maintenant si pas déjà configuré ----------------------
c_blue "[4/4] Démarrage du wizard (si pas encore configuré)"
