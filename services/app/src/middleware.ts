 ...
 *   - /api/oauth/callback (OAuth provider redirect → géré par cookie state)
 *   - /api/oauth/internal/* (workers connecteurs Python → X-Connector-Token shared secret)
...
export const config = {
  matcher: [
    "/((?!api/auth|auth/|api/agents-tools|api/oauth/callback|api/oauth/internal|api/system/health|api/version|_next/static|_next/image|favicon|robots).*)",
  ],
};
